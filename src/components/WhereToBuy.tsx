// ============================================
// DreamDesk — Блок "Где купить" (v5 — Real Data)
...
import { 
  Truck, Star, ShieldCheck, TrendingDown, Clock, Globe, MapPin,
  Circle, ChevronRight, Monitor, ShoppingBag, Zap, Tag,
  ArrowDownUp, Filter, RotateCcw, BellRing, Check, LogIn, Loader2
} from "lucide-react";
...
export function WhereToBuy({ products }: WhereToBuyProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/offers?productIds=${productIds}`)
      .then(res => res.json())
      .then(data => {
  ...
  useEffect(() => {
    ...
    fetch("/api/price-alerts")
      .then(res => res.json())
      .then(data => {
        const ids = new Set<string>((data.alerts || []).map((a: any) => a.offerId));
        setSubscribedOfferIds(ids);
      })
      .catch(() => {});
  }, [session?.user?.id]);

  // Подписка / отписка
  const toggleAlert = useCallback(async (offer: StoreOfferAPI, productName: string) => {
    if (!session?.user?.id) return;
    
    setSubscribingId(offer.id);
    const isSubscribed = subscribedOfferIds.has(offer.id);
    
    try {
      if (isSubscribed) {
        await fetch(`/api/price-alerts?offerId=${offer.id}`, { method: "DELETE" });
        setSubscribedOfferIds(prev => {
        ...
        setSubscribedOfferIds(prev => new Set(prev).add(offer.id));
      }
    } catch (err) {
  ...
  }, [session?.user?.id, subscribedOfferIds]);

  // Группировка и фильтрация предложений по продуктам
  const analyzedProducts = useMemo(() => {
    ...
    return products.map(product => {
      let productOffers = allOffers.filter(o => o.productId === product.id);

      // Применяем фильтры
      if (activeRegion !== 'all') productOffers = productOffers.filter(o => o.region === activeRegion);
      if (filterFast) productOffers = productOffers.filter(o => o.shippingFast);
      ...
      productOffers.sort((a, b) => {
        ...
        if (sortBy === 'delivery') return (a.shippingFast ? 0 : 1) - (b.shippingFast ? 0 : 1);
        if (sortBy === 'rating') return b.rating - a.rating;
        return 0;
      });

      const minPrice = productOffers.length > 0 ? Math.min(...productOffers.map(o => o.price)) : 0;
      ...
      return { product, offers: productOffers, bestPrice: minPrice, verdict };
    });
  }, [products, allOffers, activeRegion, sortBy, filterFast, filterInStock, filterOfficial]);
    ...
    <div className="space-y-16 py-20 border-t border-white/10 bg-gradient-to-b from-transparent to-dark-surface/20 -mx-4 px-4 overflow-visible">
      ...
      {loading && (
        <div className="max-w-6xl mx-auto flex items-center justify-center py-20">
          ...
          <span className="text-gray-400 text-sm font-bold">{t("loading_offers")}</span>
        </div>
      )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
                    ...
                    {offers.length > 0 ? (
                      offers.map((offer, idx) => {
                        ...
                        const isSubscribed = subscribedOfferIds.has(offer.id);
                        const isSubscribing = subscribingId === offer.id;
                        const priceHistoryValues = offer.priceHistory.map(ph => ph.price);
                          ...
                          >
                            ...
                            <div className="flex items-start justify-between mb-6">
                              ...
                              <div className="flex flex-col items-end gap-1">
                                ...
                                <span className="text-[8px] text-gray-500 uppercase font-black tracking-tighter">
                                  {offer.reviewCount.toLocaleString()} {t("reviews")}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="mb-4 space-y-1">
                              <div className="text-4xl font-black text-white tracking-tighter transition-transform duration-300 origin-left group-hover/card:scale-[1.08] group-hover/card:text-[var(--offer-brand)]">
                                {formatPrice(offer.price / EXCHANGE_RATES.EUR, currency)}
                              </div>
                              {offer.oldPrice && discount > 0 && (
                                <div className="flex items-center gap-2">
                                  <span className="text-xs text-gray-600 line-through">
                                    {formatPrice(offer.oldPrice / EXCHANGE_RATES.EUR, currency)}
                                  </span>
                                  <span className="text-[10px] text-lime font-black bg-lime/10 px-1.5 py-0.5 rounded border border-lime/20">
                                    {t("economy_percent").replace("{percent}", String(discount))}
                                  </span>
                                </div>
                              )}
                            </div>

                            {/* График цены из реальных данных */}
                            <Sparkline data={priceHistoryValues} color={offer.brandColor} t={t} />
                            ...
                            <div className="space-y-3 mt-6 mb-6 p-4 rounded-2xl bg-black/20 border border-white/5 group-hover/card:border-white/10 transition-colors">
                              <div className="flex items-center justify-between text-[11px]">
                                <div className="flex items-center gap-2 text-gray-300 font-bold">
                                  ...
                                  {offer.status === 'in_stock' ? t("filter_instock") : t("low_stock_text").replace("{count}", "3")}
                                </div>
                              </div>
                              <div className="flex flex-col gap-2 border-t border-white/10 pt-4">
                                ...
                                <div className="flex items-center gap-3 text-xs text-white">
                                  ...
                                  <span className="truncate">
                                    {offer.shippingPrice === null ? t("shipping_free") : formatPrice(offer.shippingPrice / EXCHANGE_RATES.EUR, currency)} • {offer.shippingTime}
                                  </span>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="mt-auto space-y-3">
                              <button
                                onClick={() => window.open(offer.storeUrl, "_blank")}
                              ...
                              {/* Кнопка уведомления о скидке */}
                              {session?.user ? (
                                <button 
                                  onClick={() => toggleAlert(offer, product.name)}
                                  disabled={isSubscribing}
                                  className={`w-full flex items-center justify-center gap-2 text-[10px] font-bold transition-all uppercase tracking-widest py-2 active:scale-95 ${
                                    isSubscribed 
                                      ? "text-lime bg-lime/5 hover:text-red-400 hover:bg-red-400/5"
                                      : "text-gray-500 hover:text-white hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]"
                                  }`}
                                >
                                  {isSubscribing ? (
                                ...
                                <button 
                                  ...
                                  className="w-full flex items-center justify-center gap-2 text-[10px] font-bold text-gray-500 hover:text-accent transition-all uppercase tracking-widest py-2"
                                >
                                  <LogIn className="w-3 h-3" />
                                  {t("login_required_for_alerts")}
                                </button>
                              )}
                            </div>
                          </div>
